...
def prep_model( X, Y ):
    main_input = Input( shape=( X.shape[1], X.shape[2], X.shape[3] ), name='main_input' )
    #layer = GaussianNoise(0.1)(main_input)
    layer = main_input
    layer = Conv2D( filters=11, kernel_size=(3,X.shape[2]), strides=(1,1), activation='relu', kernel_constraint=maxnorm(3.0) )(layer)
    #layer = Conv2D( filters=11, kernel_size=(3,3), strides=(1,1), activation='relu', kernel_constraint=maxnorm(3.0) )(layer)
    layer = Flatten()(layer)
    #layer = LSTM( 11, activation='relu', kernel_constraint=maxnorm(3.0), return_sequences=True )(layer)
    #layer = Dense( 256, activation='relu', kernel_constraint=maxnorm(3.0) )(layer)
...
X = numpy.zeros( [ len(T)-previous_days, previous_days, T.shape[1], 1 ] )
for t in range(len(X)):
    X[t,:,:,0] = S[t:t+previous_days,:]
Y=S[previous_days:]
...
y_predict = model.predict( X_train,  batch_size=10, verbose=1 )
y_predict = scaler.inverse_transform(y_predict)
show_errors( fechas[previous_days:N+previous_days], T[previous_days:N+previous_days], y_predict,     with_graphs=True )
show_errors( fechas[previous_days:N+previous_days-1], T[previous_days:N+previous_days-1], y_predict[1:],     with_graphs=True )
...
for t in range(len(X_test)):
    ...
    y = model.predict( x.reshape(-1,x.shape[0],x.shape[1],1), batch_size=1, verbose=0 )
    for r in range(1):
        x[:-1,:] = x[1:,:].copy()
        x[ -1,:] = y[:]
        y = model.predict( x.reshape(-1,x.shape[0],x.shape[1],1), batch_size=1, verbose=0 )
