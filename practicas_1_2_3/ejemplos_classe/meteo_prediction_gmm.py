...
K=37
gmm = GaussianMixture( n_components=K, covariance_type='full', init_params='kmeans', max_iter=2000, n_init=1 )
#gmm = GaussianMixture( n_components=K, covariance_type='full', init_params='random', max_iter=200, n_init=10 )
#gmm = BayesianGaussianMixture( n_components=K, covariance_type='full', init_params='random', max_iter=200, n_init=2 )
gmm.fit( X_train )

mean_per_class = numpy.zeros( [ K, Y_train.shape[1] ] )
denominator = numpy.zeros( K )

posteriors = gmm.predict_proba( X_train )
denominator = posteriors.sum( axis=0 )
mean_per_class = numpy.dot( posteriors.T, Y_train )
mean_per_class /= denominator.reshape(-1,1)

y_predict = numpy.zeros( [ len(Y_test), Y_test.shape[1] ] )
posteriors = gmm.predict_proba( X_test )
y_predict = numpy.dot( posteriors, mean_per_class )
